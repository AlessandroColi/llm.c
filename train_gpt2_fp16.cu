/*
GPT-2 Transformer Neural Net trained in raw CUDA - FP16 (Half Precision) Version
Based on train_gpt2_fp32.cu with modifications for reduced precision.

Key changes from FP32 version:
1. Model parameters stored in FP16 (half precision)
2. Activations stored in FP16
3. cuBLAS operations use FP16 with FP32 accumulation
4. Loss computation and some reductions kept in FP32 for stability

This version aims to:
- Reduce memory usage by ~50%
- Potentially improve performance on GPUs with FP16 support
- Evaluate accuracy tradeoffs
*/
...
// ----------------------------------------------------------------------------
// FP16 Precision Type Definitions
// Using half precision for model parameters and activations
// Note: We keep some operations (like reductions, loss) in FP32 for numerical stability

typedef half floatX;  // Main precision type for parameters and activations
#define FLOATX_SIZE sizeof(half)  // 2 bytes vs 4 bytes for float

// Helper device functions for FP16 conversion
...
// Helper for loading/storing half precision with float computation
__device__ __forceinline__ float load_as_float(const half* ptr) {
    return __half2float(*ptr);
}

__device__ __forceinline__ void store_from_float(half* ptr, float val) {
...
// ----------------------------------------------------------------------------
// FP16 cuBLAS wrapper functions for mixed precision matrix multiplication
// These use FP16 for computation but with FP32 accumulation for numerical stability

// Flag to track if we should use FP16 mode
static int use_fp16_matmul = 1;  // Set to 1 to enable FP16 matmul

// FP16 buffers for temporary conversion
static half* fp16_buffer_A = nullptr;
static half* fp16_buffer_B = nullptr;
static half* fp16_buffer_C = nullptr;
static size_t fp16_buffer_size = 0;

// Allocate FP16 buffer if needed
void ensure_fp16_buffer(size_t required_size) {
    if (fp16_buffer_size < required_size) {
        if (fp16_buffer_A) cudaFree(fp16_buffer_A);
        if (fp16_buffer_B) cudaFree(fp16_buffer_B);
        if (fp16_buffer_C) cudaFree(fp16_buffer_C);
        cudaCheck(cudaMalloc(&fp16_buffer_A, required_size * sizeof(half)));
        cudaCheck(cudaMalloc(&fp16_buffer_B, required_size * sizeof(half)));
        cudaCheck(cudaMalloc(&fp16_buffer_C, required_size * sizeof(half)));
        fp16_buffer_size = required_size;
    }
}

// Kernel to convert float to half
__global__ void float_to_half_kernel(half* out, const float* in, int n) {
...
void convert_float_to_half(half* out, const float* in, int n) {
    int block_size = 256;
    int grid_size = CEIL_DIV(n, block_size);
    float_to_half_kernel<<<grid_size, block_size>>>(out, in, n);
    cudaCheck(cudaGetLastError());
}

// Convert half array to float
void convert_half_to_float(float* out, const half* in, int n) {
    int block_size = 256;
    int grid_size = CEIL_DIV(n, block_size);
    half_to_float_kernel<<<grid_size, block_size>>>(out, in, n);
    cudaCheck(cudaGetLastError());
}

// Mixed precision SGEMM: inputs in FP32, compute in FP16, output in FP32
void cublasSgemm_fp16(cublasHandle_t handle, cublasOperation_t transa, cublasOperation_t transb,
    ...
    // Calculate sizes
    int size_A = (transa == CUBLAS_OP_N) ? m * k : k * m;
    int size_B = (transb == CUBLAS_OP_N) ? k * n : n * k;
    int size_C = m * n;
    int max_size = max(max(size_A, size_B), size_C);
    
    // Ensure we have enough buffer space
    ensure_fp16_buffer(max_size);
    
    // Convert inputs to FP16
    convert_float_to_half(fp16_buffer_A, A, size_A);
    convert_float_to_half(fp16_buffer_B, B, size_B);
    if (*beta != 0.0f) {
        convert_float_to_half(fp16_buffer_C, C, size_C);
    }
    ...
    cublasCheck(cublasGemmEx(handle, transa, transb, m, n, k,
                             &alpha_h, fp16_buffer_A, CUDA_R_16F, lda,
                             fp16_buffer_B, CUDA_R_16F, ldb,
                             &beta_h, fp16_buffer_C, CUDA_R_16F, ldc,
                             CUBLAS_COMPUTE_16F, CUBLAS_GEMM_DEFAULT));
    
    // Convert output back to FP32
    convert_half_to_float(C, fp16_buffer_C, size_C);
}

// Mixed precision strided batched SGEMM
void cublasSgemmStridedBatched_fp16(cublasHandle_t handle, cublasOperation_t transa, cublasOperation_t transb,
                                     int m, int n, int k, const float* alpha,
    ...
    if (!use_fp16_matmul) {
        // Fall back to FP32
        cublasCheck(cublasSgemmStridedBatched(handle, transa, transb, m, n, k, alpha, 
                    A, lda, strideA, B, ldb, strideB, beta, C, ldc, strideC, batchCount));
        return;
    }
    
    // For strided batched, we need larger buffers
    int size_A = (transa == CUBLAS_OP_N) ? m * k : k * m;
    int size_B = (transb == CUBLAS_OP_N) ? k * n : n * k;
    int size_C = m * n;
    int total_A = size_A * batchCount;
    int total_B = size_B * batchCount;
    int total_C = size_C * batchCount;
    int max_size = max(max(total_A, total_B), total_C);
    
    ensure_fp16_buffer(max_size);
    
    // Convert all batches
    convert_float_to_half(fp16_buffer_A, A, total_A);
    convert_float_to_half(fp16_buffer_B, B, total_B);
    if (*beta != 0.0f) {
        convert_float_to_half(fp16_buffer_C, C, total_C);
    }
    
    __half alpha_h = __float2half(*alpha);
    __half beta_h = __float2half(*beta);
    
    cublasCheck(cublasGemmStridedBatchedEx(handle, transa, transb, m, n, k,
                                           &alpha_h, fp16_buffer_A, CUDA_R_16F, lda, strideA,
                                           fp16_buffer_B, CUDA_R_16F, ldb, strideB,
                                           &beta_h, fp16_buffer_C, CUDA_R_16F, ldc, strideC,
                                           batchCount, CUBLAS_COMPUTE_16F, CUBLAS_GEMM_DEFAULT));
    
    convert_half_to_float(C, fp16_buffer_C, total_C);
}

// Cleanup function for FP16 buffers
void cleanup_fp16_buffers() {
    if (fp16_buffer_A) { cudaFree(fp16_buffer_A); fp16_buffer_A = nullptr; }
    if (fp16_buffer_B) { cudaFree(fp16_buffer_B); fp16_buffer_B = nullptr; }
    if (fp16_buffer_C) { cudaFree(fp16_buffer_C); fp16_buffer_C = nullptr; }
    fp16_buffer_size = 0;
}
...
void attention_forward(float* out, float* qkvr, float* att,
    ...
    // batched matrix multiply with cuBLAS (FP16 version)
    const float alpha = 1.0f;
    const float beta = 0.0f;
    float* preatt = inp;
    cublasSgemmStridedBatched_fp16(cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, T, T, HS, &alpha, k, HS, T * HS, q, HS, T * HS, &beta, preatt, T, T * T, B * NH);
    ...
    // new approach: first cuBLAS another batched matmul (FP16 version)
    float* vaccum = inp;
    // y = att @ v # (B, nh, T, T) @ (B, nh, T, hs) -> (B, nh, T, hs)
    cublasSgemmStridedBatched_fp16(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, HS, T, T, &alpha, v, HS, T * HS, att, T, T * T, &beta, vaccum, HS, T * HS, B * NH);
...
void matmul_backward(float* dinp, float* dweight, float* dbias,
    ...
    float one = 1.0f;
    float zero = 0.0f;
    // backward to input, uses = in the backward pass (set the gradient) - FP16 version
    cublasSgemm_fp16(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, C, B*T, OC, &one, weight, C, dout, OC, &zero, dinp, C);
    // backward to weight, uses += in the backward pass (accumulate the gradient) - FP16 version
    cublasSgemm_fp16(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_T, C, OC, B*T, &one, inp, C, dout, OC, &one, dweight, C);
...
void attention_backward(float* dinp, float* dqkvr, float* dpreatt, float* datt, float* scratch,
    ...
    cudaCheck(cudaGetLastError());
    // backward into datt - FP16 version
    cublasSgemmStridedBatched_fp16(cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, T, T, HS, &one, v, HS, T * HS, scratch, HS, T * HS, &zero, datt, T, T * T, B * NH);
    // backward into dv - FP16 version
    cublasSgemmStridedBatched_fp16(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_T, HS, T, T, &one, scratch, HS, T * HS, att, T, T * T, &zero, dv, HS, T * HS, B * NH);
    // backward into preatt
    int hs = C / NH; // head size
    float scale = 1.0f / sqrtf(hs);
    softmax_autoregressive_backward_kernel<<<dim3(T / 4, B * NH), 256>>>(dpreatt, datt, att, B, T, C, scale);
    cudaCheck(cudaGetLastError());
    // backward into q - FP16 version
    cublasSgemmStridedBatched_fp16(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, HS, T, T, &one, k, HS, T * HS, dpreatt, T, T * T, &zero, dq, HS, T * HS, B * NH);
    // backward into k - FP16 version
    cublasSgemmStridedBatched_fp16(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_T, HS, T, T, &one, q, HS, T * HS, dpreatt, T, T * T, &zero, dk, HS, T * HS, B * NH);
    // backward into inp
...
int main(int argc, char *argv[]) {
    ...
    printf("+-----------------------+----------------------------------------------------+\n");
    printf("| FP16 MIXED PRECISION VERSION                                               |\n");
    printf("+-----------------------+----------------------------------------------------+\n");
    ...
    printf("| genT                  | %-50d |\n", genT);
    printf("| FP16 matmul           | %-50s |\n", use_fp16_matmul ? "enabled" : "disabled");
    printf("+-----------------------+----------------------------------------------------+\n");
    ...
    free(cpu_logits);
    free(gen_tokens);
    cleanup_fp16_buffers();  // Clean up FP16 conversion buffers
